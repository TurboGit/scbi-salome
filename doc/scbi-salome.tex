...
\cmd{scbi \ddash{}deps \ddash{}env=v96 salome}
...
\cmd{scbi \ddash{}deps \ddash{}env=v96 \ddash{}enable-installer salome}
...
\cmd{scbi \ddash{}deps \ddash{}plan=v96 salome}
...
\cmdc{paraview/gdal.ospray:2a6cc49e5f salome}
...
To build some modules on top of a binary installation the specific plug-in \code{s-salome-bin} is provided.

Also two variants are introduced:

\begin{enumerate}
	\item \code{ng} : to build on top of a Linux binary installation.
	\item \code{ngwin} : to build on top of a Windows binary installation.
\end{enumerate}

\subsection{Plug-in s-salome-bin}
...
Taking the example of the \code{atomic} plug-in:

\begin{itemize}
	\item Rename \code{atomic-build-depends} hook to \code{atomic-common-build-depends}. This is to share the hook with the standard build.

	\item Rename \code{atomic-depends} hook to \code{atomic-default-depends}. This is to use the dependencies only for the standard variant and not with the \code{ng} one.

	\item Add \code{atomic-linux-build-depends} hook:
	\begin{lstlisting}
	function atomic-ng-build-depends()
	{
		echo s-salome-bin
	}
	\end{lstlisting}
...
If the module contains a more complex setup with already multiple variants for the configure options the setup may be a bit more challenging than for the above \code{atomic} module. Note that in any cases only the \code{depends}, \code{build-depends} and \code{config-options} hooks need attention. At the end the \code{common} plus \code{ng} variants of the above hook are what will be used for the \code{ng} variant, and the \code{common}, plus \code{default} hooks must be equivalent to previous version for the non ng build.

The followings modules have \code{ng} variants and can be used as source of inspiration:
...
\subsection{Plug-ins s-windows-kit and s-visual-c}
...
A good example is the \code{paravisaddons} plug-in which can be used to build paravisaddons as part of a full build, on top of a SALOME Linux installation or on top of a SALOME Windows installation.
