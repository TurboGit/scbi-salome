...
\begin{description}
	...
	\item[PATTERN-N] A regular expression of the module as found in the binary archive. It is used to remove this from the NG environment by matching the variable name and the variable values (path).
\end{description}

For example, to build SMESH and PERSALYS on top of binary archive and integrate them into the final application:

\cmd{export SCBI\_EXTRA\_NG\_MODULES="s-persalys(PERSALYS):s-smesh(SMESH)"}
