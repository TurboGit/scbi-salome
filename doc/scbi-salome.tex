...
\begin{enumerate}
	\item \code{linux} : to build on top of a Linux binary installation.
	\item \code{win} : to build on top of a Windows binary installation.
\end{enumerate}
...
\begin{enumerate}
	\item Set \code{SCBI\_SAOME\_BIN} to point to the SALOME root installation directory.
	\item Add \code{s-salome-bin} in module's build-depends hook.
	\item Update the \code{build-depends} and \code{depends} hook to take the new variant into account.
\end{enumerate}
...
\begin{itemize}
	...
	\item Rename \code{atomic-depends} hook to \code{atomic-default-depends}. This is to use the dependencies only for the standard variant and not with the linux one.

	\item Add \code{atomic-linux-build-depends} hook:
	\begin{lstlisting}
	function atomic-linux-build-depends()
	{
		echo s-salome-bin
	}
	\end{lstlisting}
